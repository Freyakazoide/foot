...
async function handleEndOfMonthEvents(db) { // Não precisa mais do playerClubId, podemos buscar aqui
    try {
...
function registerGameLoopHandlers(db) { // <-- Aceita a conexão 'db'
    const dbGet = util.promisify(db.get.bind(db));
    ...
    ipcMain.handle('advance-time', async (event, { currentDate }) => {
        try {
            ...
            if (nextDate.getUTCMonth() !== previousDate.getUTCMonth()) {
                await handleEndOfMonthEvents(db);
            }
            ...
            console.error("Erro em 'advance-time':", err);
            throw err;
        }
    });
    
    // O 'db' aqui agora vem do parâmetro da função registerGameLoopHandlers
